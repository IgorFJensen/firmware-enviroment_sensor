...
static float dps_compensate_temp(int32_t raw) {
    // Usando o novo fator de escala para oversampling máximo
    return calib.c0 * 0.5f + calib.c1 * raw / 524288.0f;
}

static float dps_compensate_pressure(int32_t raw_p, int32_t raw_t) {
    // Usando o novo fator de escala para oversampling máximo para ambos
...
esp_err_t dps310_init(i2c_master_bus_handle_t bus) {
    ...
    // Configura sensor para modo contínuo temperatura+pressão
    // Temperatura oversampling 128x (TMP_EN=1, TMP_PRC=111) -> 0b11110000 = 0xF0
    dps_write_reg(DPS310_REG_TMP_CFG, 0x80);
    dps_write_reg(DPS310_REG_PSR_CFG, 0x01);
...
esp_err_t dps310_read(float *temperature, float *pressure) {
    ...
    int32_t raw_temp = dps_read_raw24(DPS310_REG_TMP_B2);
    int32_t raw_pres = dps_read_raw24(DPS310_REG_PSR_B2);
