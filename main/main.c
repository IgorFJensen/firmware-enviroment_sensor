...
void app_main(void)
{
    ...
    switch (wakeup_cause) {
        case ESP_SLEEP_WAKEUP_TIMER:
            ESP_LOGI(TAG, "Acordado por timer de deep sleep");
            break;
        case ESP_SLEEP_WAKEUP_UNDEFINED:
            ESP_LOGI(TAG, "Boot normal (não foi deep sleep)");
            break;
        default:
            ESP_LOGI(TAG, "Boot com causa de wakeup: %d", wakeup_cause);
            break;
    }
    ...
    //GPIO 19 para ligar os sensores, GPIO 15 para ligar o led
    gpio_reset_pin(19);
    gpio_reset_pin(15);
